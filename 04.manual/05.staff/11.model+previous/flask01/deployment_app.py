...
unique_projects = staff_data['Project'].unique()
...
def index():
    if request.method == 'POST':
        # Check if the request is for generating deployment plan
        if 'project_type' in request.form:
            project_type = request.form['project_type']
            duration = int(request.form['duration'])
            additional_input = request.form.get('additionalInputField', None)
            deployment_plan_html = generate_deployment_plan(project_type, duration, additional_input)
            
            # Transpose the DataFrame
            transposed_plan = deployment_plan_html.T
            ...
            return render_template('index.html', deployment_plan_html=transposed_plan.to_html(), unique_projects=unique_projects)
        
        # Check if the request is for viewing deployment plan for previous project
        elif 'previous_project' in request.form:
            selected_project = request.form['previous_project']
            # Fetch deployment plan for selected previous project (You need to implement this)
            # deployment_plan_html = fetch_deployment_plan_for_previous_project(selected_project)
            deployment_plan_html = "<h2>Deployment Plan for Previous Project: {}</h2>".format(selected_project)
            return render_template('index.html', deployment_plan_html=deployment_plan_html, unique_projects=unique_projects)
    # Render the index.html template with unique projects for the dropdown
    return render_template('index.html', unique_projects=unique_projects.tolist())
