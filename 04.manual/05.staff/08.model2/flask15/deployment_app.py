...
def index():
    ...
    if request.method == 'POST':
        ...
        # Retain the bridge length if the project type is "Bridges"
        if project_type == 'Bridges':
            bridge_length = request.form['bridge_length']
        elif project_type in ['Commercial', 'Offices & Mall']:
            # Retrieve the additional input field value if project type requires it
            additional_input = request.form.get('additionalInputField', '')
        ...
        return render_template('index.html', deployment_plan_html=transposed_plan.to_html(), bridge_length=bridge_length)
    else:
        # Render the form template if no data has been submitted
        return render_template('form.html', project_type=request.form.get('project_type', 'Bridges'), duration=request.form.get('duration', ''), additional_input=request.form.get('additionalInputField', ''))
