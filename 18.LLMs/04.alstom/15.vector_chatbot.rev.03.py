...
def process_question(message, history, doc_paths_state):
    try:
        ...
        for i, doc in enumerate(relevant_docs):
            ...
            # Replace C:\alstom with network path
            network_path = doc_path.replace("C:\\alstom", "\\\\fileserver2\\Head Office Server\\Projects Control (PC)\\10 Backup\\05 Models\\alstom")
            
            doc_buttons_html += f"<button onclick=\"window.open('file:///{network_path.replace('\\', '/')}', '_blank')\" style='margin: 5px; padding: 5px 10px; background-color: #f0f0f0; border: 1px solid #ddd; border-radius: 4px; cursor: pointer;'>{doc_name}</button>"
