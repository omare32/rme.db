...
def get_random_rows(conn, n=5):
    with conn.cursor() as cursor:
        cursor.execute(f"SELECT id FROM `{TABLE}` WHERE extracted_text IS NOT NULL AND LENGTH(extracted_text) > 20 AND pdf_path IS NOT NULL AND pdf_path != '' AND document_type = 'Purchase Order'")
...
def create_cover_pdf(title, answer, output_path):
    ...
    # Shape the title for Arabic (in case the question/label is Arabic)
...
def main():
    ...
    for i, row in enumerate(rows, 1):
        ...
        # Diagnostic: Check type and content of extracted_text
        print(f"\n[DIAG] Row {i} (ID: {pdf_id}) extracted_text type: {type(text)}")
        if isinstance(text, bytes):
            try:
                text = text.decode('utf-8')
                print("[DIAG] Decoded extracted_text from bytes.")
            except Exception as e:
                print(f"[DIAG] Could not decode extracted_text: {e}")
        print(f"[DIAG] extracted_text (first 200 chars): {repr(text[:200])}")
        cover_pdfs = []
        answers = {}
        ...
        for model_name, model_id in OLLAMA_MODELS:
            for q_title, prompt_template in QUESTIONS:
                ...
                print(f"\n[DIAG] Prompt for {model_name} - {q_title} type: {type(prompt)}")
                print(f"[DIAG] Prompt content (first 200 chars): {repr(prompt[:200])}")
                print(f"Prompt for {model_name} - {q_title}:\n{'-'*40}\n{prompt}\n--- SENDING TO LLM ---")
                ...
                print(f"LLM response for {model_name} - {q_title}:\n{'-'*40}\n{llm_result}\n")
                answers[f"{model_name}_{q_title}"] = llm_result
                cover_pdf_path = os.path.join(OUTPUT_DIR, f"cover_{pdf_id}_{model_name}_{q_title.replace(' ', '_')}.pdf")
                create_cover_pdf(f"{model_name}: {q_title}", llm_result, cover_pdf_path)
                cover_pdfs.append(cover_pdf_path)
        # 2. OCR extracted text (1+ pages)
        ocr_pdf_path = os.path.join(OUTPUT_DIR, f"ocr_text_{pdf_id}.pdf")
        create_text_pdf("Extracted OCR Text", text, ocr_pdf_path)
        cover_pdfs.append(ocr_pdf_path)
        ...
        debug_log.append({'id': pdf_id, 'answers': answers, 'ocr_text': text})
    # Save debug log
