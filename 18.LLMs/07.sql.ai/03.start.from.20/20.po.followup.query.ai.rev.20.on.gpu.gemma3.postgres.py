...
def fix_sql_query_order(sql_query: str) -> str:
    """
    Corrects the order of clauses in a SQL query, moving WHERE before ORDER BY and LIMIT.
    For example, `SELECT * FROM table ORDER BY col LIMIT 5 WHERE condition` becomes
    `SELECT * FROM table WHERE condition ORDER BY col LIMIT 5`.
    """
    sql_lower = sql_query.lower()
    
    # Find positions of the clauses, with spaces to avoid matching substrings
    where_pos = sql_lower.find(" where ")
    order_pos = sql_lower.find(" order by ")
    limit_pos = sql_lower.find(" limit ")

    # Proceed only if a WHERE clause exists
    if where_pos == -1:
        return sql_query

    # Find the first position of an ordering/limiting clause
    order_limit_positions = [p for p in [order_pos, limit_pos] if p != -1]
    first_order_limit_pos = min(order_limit_positions) if order_limit_positions else -1

    # If WHERE is misplaced (i.e., it comes after an ORDER BY or LIMIT)
    if first_order_limit_pos != -1 and where_pos > first_order_limit_pos:
        # Extract the parts of the query
        main_part = sql_query[:first_order_limit_pos]
        order_limit_part = sql_query[first_order_limit_pos:where_pos]
        where_part = sql_query[where_pos:]
        
        # Reassemble the query in the correct order
        corrected_query = f"{main_part.strip()} {where_part.strip()} {order_limit_part.strip()}"
        
        # For debugging purposes
        print(f"--- SQL Query Correction ---")
...
def create_interface():
    with gr.Blocks(title="RME PO Query Assistant rev.20 (PostgreSQL with Memory)") as interface:
        # Title only at the top - moved descriptive text to bottom
        gr.Markdown("# PO Follow-Up Query AI (rev.20, PostgreSQL with Memory)")
