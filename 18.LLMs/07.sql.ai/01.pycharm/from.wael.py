...
SYSTEM_STATS = {
    ...
    "decomposed_questions": 0,
    "entity_links_discovered": 0
}
...
class ConversationManager:
    def __init__(self, max_history=5):
        ...
        self.entity_attributes = {}    # Track attributes of entities
        self.last_query_result = None
        self.last_sql_query = None
        self.last_question = None
        self.previous_queries = set()  # Track unique queries for duplicate detection
        ...
        self.failed_attempts = {}  # Track failed attempts for specific entity+query combinations
    ...
    def add_interaction(self, question: str, answer: str, entities: Dict, 
            ...
            if entity_type in ["project", "supplier"] and entity_value:
                ...
                if "project" in self.detected_entities and "supplier" in self.detected_entities:
                    ...
                    if project not in self.entity_relationships["projects_by_supplier"][supplier]:
                        ...
                        SYSTEM_STATS["entity_links_discovered"] += 1
                ...
                # Track entity attributes if found in results
                if query_result and "results" in query_result and query_result["results"]:
                    # Initialize entity attribute tracking
                    if entity_type not in self.entity_attributes:
                        self.entity_attributes[entity_type] = {}
                    if entity_value not in self.entity_attributes[entity_type]:
                        self.entity_attributes[entity_type][entity_value] = {}

                    # Extract attributes from query results if they match our entity
                    if "columns" in query_result and query_result["columns"]:
                        columns = query_result["columns"]

                        # Look for the entity column index
                        entity_col_idx = -1
                        if entity_type == "project":
                            entity_col_idx = next((i for i, col in enumerate(columns) if col.upper() == "PROJECT_NAME"), -1)
                        elif entity_type == "supplier":
                            entity_col_idx = next((i for i, col in enumerate(columns) if col.upper() == "VENDOR_NAME"), -1)

                        # If we found the entity column, look for matching rows
                        if entity_col_idx >= 0:
                            for row in query_result["results"]:
                                if entity_col_idx < len(row) and row[entity_col_idx] == entity_value:
                                    # Extract other attributes from this row
                                    for i, col in enumerate(columns):
                                        if i != entity_col_idx and i < len(row) and row[i] is not None:
                                            # Store the attribute
                                            attr_name = col.lower()
                                            self.entity_attributes[entity_type][entity_value][attr_name] = row[i]
        ...
        if sql_query:
            ...
            # Add to previous queries set for duplicate detection
            self.previous_queries.add(self._normalize_query(sql_query))
    ...
    def _normalize_query(self, query):
        """Normalize a SQL query for comparison (removes whitespace variations)"""
        if not query:
            return ""
    ...
    def is_duplicate_query(self, query):
        """Check if this query is a duplicate of a previous one"""
        normalized = self._normalize_query(query)
        return normalized in self.previous_queries
    ...
    def get_context_for_llm(self) -> str:
        ...
        # Add entity attributes if we have them
        if self.entity_attributes:
            context += "\nEntity attributes:\n"
            for entity_type, entities in self.entity_attributes.items():
                for entity_name, attributes in entities.items():
                    if attributes:  # Only add if we have attributes
                        # Get the top 3 most valuable attributes
                        top_attrs = [(k, v) for k, v in attributes.items() 
                                    if k not in ["project_name", "vendor_name"] and v is not None][:3]
                        if top_attrs:
                            attrs_str = ", ".join([f"{k}: {v}" for k, v in top_attrs])
                            context += f"{entity_type.capitalize()} '{entity_name}' has {attrs_str}\n"
    ...
    def get_entity_attributes(self) -> Dict:
    ...
    def record_failed_attempt(self, entity_type, entity_value, query_hash):
        """Record a failed attempt for this entity and query combination"""
        key = f"{entity_type}:{entity_value}:{query_hash}"
        if key not in self.failed_attempts:
            self.failed_attempts[key] = 1
        else:
            self.failed_attempts[key] += 1

    def get_failed_attempts(self, entity_type, entity_value, query_hash):
        """Get number of failed attempts for this entity and query combination"""
        key = f"{entity_type}:{entity_value}:{query_hash}"
        ...
        if json_match:
            ...
            try:
                ...
                if "sub_questions" in decomposition and isinstance(decomposition["sub_questions"], list):
                    ...
                    # Update stats if we successfully decomposed
                    if len(sub_questions) > 1:
                        SYSTEM_STATS["decomposed_questions"] += 1
...
def detect_entities_with_llm(question: str, use_history: bool = True) -> dict:
    ...
    """
    try:
        # Calculate cache key for this request
        cache_key = f"entity_detection:{question}:{use_history}"
        if use_history:
            # Include a hash of the conversation history in the cache key
            history_hash = hashlib.md5(str(CONVERSATION.conversation_history).encode()).hexdigest()[:10]
            cache_key += f":{history_hash}"
        ...
        result = {"project": None, "supplier": None}
        QUERY_CACHE[cache_key] = result
        return result
...
def generate_clarification_questions(question: str, detected_entities: dict) -> List[str]:
    ...
    if not clarification_questions and (not detected_entities.get("project") or not detected_entities.get("supplier")):
        try:
            # Calculate cache key for this request
            cache_key = f"clarification:{question}:{detected_entities.get('project')}:{detected_entities.get('supplier')}"
            ...
            if suggestion and "NO_CLARIFICATION_NEEDED" not in suggestion:
                ...
                # Cache the result
                QUERY_CACHE[cache_key] = clarification_questions
...
def process_question(question: str, use_history: bool = True) -> Tuple[str, str, List[str], List[List], Dict]:
    ...
    # Check for empty or trivial questions
    if not question or question.strip() in ["?", ".", ""]:
        return ("Please ask a more specific question about purchase orders.", 
                "", ["Input Error"], [["Empty or invalid question"]], {})
...
def process_single_question(question: str, use_history: bool = True) -> Tuple[str, str, List[str], List[List], Dict]:
    ...
    """
    try:
        # Calculate a simple hash of the question to track failed attempts
        ...
        # Track failed attempts for entity-query combinations
        if detected_entities["project"]:
            failed_attempts = CONVERSATION.get_failed_attempts("project", detected_entities["project"], question_hash)
            if failed_attempts > 2:
                # After multiple failures with this entity, try without it
                print(f"Multiple failures with project '{detected_entities['project']}', trying without it")
                detected_entities["project"] = None
                detected_entities["project_confidence"] = 0.0

        if detected_entities["supplier"]:
            failed_attempts = CONVERSATION.get_failed_attempts("supplier", detected_entities["supplier"], question_hash)
            if failed_attempts > 2:
                # After multiple failures with this entity, try without it
                print(f"Multiple failures with supplier '{detected_entities['supplier']}', trying without it")
                detected_entities["supplier"] = None
                detected_entities["supplier_confidence"] = 0.0
        ...
        if CONVERSATION.is_duplicate_query(sql_query):
            # Generate a slightly different query to avoid duplicates
            if "LIMIT" in sql_query.upper():
                # Modify the LIMIT to get a different result set
                sql_query = re.sub(r'LIMIT\s+\d+', 'LIMIT 10', sql_query, flags=re.IGNORECASE)
            else:
                # Add a comment to make the query unique
                sql_query = f"-- Re-executing similar query\n{sql_query}"

        # Execute query
        try:
            # Check query cache first (if not a mutating query)
            if sql_query.strip().upper().startswith("SELECT") or sql_query.strip().upper().startswith("WITH"):
                cache_key = f"query:{sql_query}"
                if cache_key in QUERY_CACHE:
                    print("Using cached query result")
                    columns, results = QUERY_CACHE[cache_key]
                else:
                    columns, results = execute_query(sql_query)
                    # Cache the result for future use
                    QUERY_CACHE[cache_key] = (columns, results)
            else:
                # Execute non-SELECT query without caching
                columns, results = execute_query(sql_query)

            query_result = {"columns": columns, "results": results}

            if columns[0] == "Error":
                # Record the failed attempt for this entity-query combination
                if detected_entities["project"]:
                    CONVERSATION.record_failed_attempt("project", detected_entities["project"], question_hash)
                if detected_entities["supplier"]:
                    CONVERSATION.record_failed_attempt("supplier", detected_entities["supplier"], question_hash)
                ...
                if fixed_query and fixed_query != sql_query:
                    ...
                    if columns[0] != "Error":
                        ...
                        SYSTEM_STATS["successful_recoveries"] += 1
                    else:
                ...
                answer = generate_natural_language_answer(question, sql_query, columns, results, detected_entities, use_history)
...
def optimize_query_for_performance(sql_query: str) -> str:
    ...
    # Optimization 4: Avoid full table scans for large result sets
    if "GROUP BY" not in query_upper and "WHERE" not in query_upper and "LIMIT" not in query_upper:
        # Query might return the entire table
        sql_query += " LIMIT 1000"  # Add safety limit
...
def recover_from_query_error(error: str, original_query: str, question: str) -> str:
    ...
    if "syntax error at or near" in error_lower:
        ...
        try:
            ...
            system_message += "\n\nImportant notes:"
            system_message += "\n1. PostgreSQL column names must be in double quotes: \"COLUMN_NAME\""
            system_message += "\n2. The table name is 'po_followup_merged'"
            system_message += "\n3. Common columns: \"PO_NUM\", \"PROJECT_NAME\", \"VENDOR_NAME\", \"TERMS\", \"APPROVED_DATE\", \"UNIT_PRICE\""
            ...
            # Sanity check: ensure we're returning a valid query
            if ("SELECT" in fixed_query.upper() or "WITH" in fixed_query.upper()) and "FROM" in fixed_query.upper():
                return fixed_query
    ...
    # Fix 4: Invalid character in string literal
    if "unterminated quoted string" in error_lower or "unterminated quoted identifier" in error_lower:
        # Try to fix quote issues
        try:
            # Find potential unclosed quotes
            quote_positions = [m.start() for m in re.finditer('"', original_query)]
            if len(quote_positions) % 2 == 1:  # Odd number of quotes indicates unclosed quote
                # Add a missing quote at the end of a likely word
                last_space_after_quote = original_query.rfind(' ', quote_positions[-1])
                if last_space_after_quote > quote_positions[-1]:
                    fixed_query = original_query[:last_space_after_quote] + '"' + original_query[last_space_after_quote:]
                    return fixed_query
                else:
                    # Add at the end of the query if no better place found
                    return original_query + '"'
        except:
            pass

    # Fix 5: Missing FROM clause
    if "missing FROM-clause" in error_lower:
        if "SELECT" in original_query.upper() and "FROM" not in original_query.upper():
            # Add the missing FROM clause
            fixed_query = original_query.replace("SELECT", "SELECT * FROM po_followup_merged WHERE", 1)
            fixed_query = fixed_query.replace("select", "SELECT * FROM po_followup_merged WHERE", 1)
            return fixed_query
...
def generate_sql_query(question: str, detected_entities: dict = None, use_history: bool = True) -> str:
    try:
        ...
        # Calculate cache key for this request
        cache_key = f"sql_gen:{question}:{detected_entities.get('project')}:{detected_entities.get('supplier')}"
        if use_history:
            # Include a hash of the conversation history in the cache key
            history_hash = hashlib.md5(str(CONVERSATION.conversation_history).encode()).hexdigest()[:10]
            cache_key += f":{history_hash}"
        ...
        if use_history and CONVERSATION.conversation_history:
            ...
            # Add entity attributes for better query generation
            entity_attributes = CONVERSATION.get_entity_attributes()
            if entity_attributes:
                context_message += "\nKnown entity attributes:\n"
                for entity_type, entities in entity_attributes.items():
                    for entity_name, attributes in entities.items():
                        if attributes:  # Only add if we have attributes
                            attrs_str = ", ".join([f"{k}: {v}" for k, v in list(attributes.items())[:3]])
                            context_message += f"{entity_type.capitalize()} '{entity_name}' has {attrs_str}\n"
        ...
        # Cache the result
        QUERY_CACHE[cache_key] = sql_query
        return sql_query
    except Exception as e:
...
def determine_optimal_response_format(question: str, columns: list, results: list) -> str:
    ...
    # Check for trends or time-based analysis
    trend_terms = ["trend", "over time", "pattern", "history", "change", "growth"]
    is_trend_analysis = any(term in question_lower for term in trend_terms)

    # Check for comparison questions
    comparison_terms = ["compare", "versus", "vs", "difference", "between", "against"]
    is_comparison = any(term in question_lower for term in comparison_terms)
    ...
    if is_analytical and result_count == 1 and column_count <= 3:
        ...
        return "trend_summary"  # Trend analysis with potential chart suggestion
    elif is_comparison:
        return "comparison_summary"  # Comparison with key differences highlighted
...
def generate_natural_language_answer(question: str, sql_query: str, columns: list, results: list, 
                                  detected_entities: dict, use_history: bool = True) -> str:
    if not results or len(results) == 0:
    ...
    if columns[0] == "Error":
        ...
        if response_format == "table" or response_format == "empty":
            ...
            result_str += f"{header}\n{sep}\n" + "\n".join(sample_rows)
        elif response_format == "trend_summary":
            result_str = f"Trend analysis results ({len(results)} data points):\n\n"
            header = " | ".join(columns)
            sep = " | ".join(["---"] * len(columns))
            sample_rows = [" | ".join(str(cell) for cell in row) for row in results[:5]]
            result_str += f"{header}\n{sep}\n" + "\n".join(sample_rows)
            result_str += "\n\n[Trend analysis data available - consider visualizing as a chart]"
        elif response_format == "comparison_summary":
            result_str = f"Comparison results ({len(results)} items):\n\n"
            header = " | ".join(columns)
            sep = " | ".join(["---"] * len(columns))
            sample_rows = [" | ".join(str(cell) for cell in row) for row in results[:5]]
            result_str += f"{header}\n{sep}\n" + "\n".join(sample_rows)
            result_str += "\n\n[Comparison data available - key differences highlighted in answer]"
        else:  # summary format
    ...
    # Calculate cache key for this request
    cache_key = f"answer:{question}:{sql_query}:{response_format}"
    question_hash = hashlib.md5(question.encode()).hexdigest()[:8]
    result_hash = hashlib.md5(str(results[:3]).encode()).hexdigest()[:8]
    cache_key = f"answer:{question_hash}:{result_hash}:{response_format}"

    if use_history:
        # Include a hash of the conversation history in the cache key
        history_hash = hashlib.md5(str(CONVERSATION.conversation_history[-2:]).encode()).hexdigest()[:10]
        cache_key += f":{history_hash}"
    ...
    # Add specific guidance based on response format
    if response_format == "trend_summary":
        prompt += "\n\nThis is a trend analysis question. Highlight changes over time, patterns, or notable trends in the data."
    elif response_format == "comparison_summary":
        prompt += "\n\nThis is a comparison question. Highlight key differences between the items being compared."
    elif response_format == "summary_with_examples":
        prompt += "\n\nThis has many results. Summarize key patterns and provide insights from the sample data."
...
def clear_query_cache():
    ...
    QUERY_CACHE = {}
    print(f"Query cache cleared. {cache_size} entries removed.")
    return cache_size

def create_interface():
    with gr.Blocks(title="RME PO Query Assistant rev.18 (Enhanced PostgreSQL with Advanced Processing)") as interface:
        # Title and version info
        gr.Markdown(f"# PO Follow-Up Query AI (rev.18, PostgreSQL with Advanced Intelligence)")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                ...
                with gr.Row():
                    ...
                    cache_status = gr.Markdown("Cache: 0 entries")
        ...
        gr.Markdown(
            "### System Information\n"
            "This advanced AI assistant queries the merged PO table with sophisticated question understanding.\n\n"
            ...
            "- Uses adaptive query generation and error recovery\n"
            "- Implements caching for faster repeat queries\n"
            "- Remembers entity attributes for improved context\n"
        )
        ...
        def on_submit(question, chat_history):
            ...
            try:
                ...
                if entity_relationships and ("suppliers_by_project" in entity_relationships or "projects_by_supplier" in entity_relationships):
                    ...
                    if entity_attributes:
                        ...
                        for entity_type, entities in entity_attributes.items():
                            for entity_name, attributes in entities.items():
                                if attributes:  # Only add if we have attributes
                                    # Show top 2 most valuable attributes
                                    top_attrs = [(k, v) for k, v in attributes.items() 
                                                if k not in ["project_name", "vendor_name"] and v is not None][:2]
                                    if top_attrs:
                                        attrs_str = ", ".join([f"{k}: {v}" for k, v in top_attrs])
                                        relationships_markdown += f"**{entity_name}** - {attrs_str}\n"
                else:
                ...
                if query.startswith("Error") or (columns and columns[0] == "Error"):
                    ...
                    return chat_history, "", entities_markdown, query, gr.Dataframe(value=[[error_message]], headers=["Error"]), complexity_label, relationships_markdown, update_system_info(), f"Cache: {len(QUERY_CACHE)} entries"

                # Create dataframe for results
                df = gr.Dataframe(value=results_data, headers=columns)
                return chat_history, "", entities_markdown, query, df, complexity_label, relationships_markdown, update_system_info(), f"Cache: {len(QUERY_CACHE)} entries"
        ...
        def update_system_info():
            return (f"### System Stats\n" + 
                   ...
                   f"- Recoveries: {SYSTEM_STATS['successful_recoveries']}\n" +
                   f"- Decomposed: {SYSTEM_STATS['decomposed_questions']}\n" +
                   f"- Entity Links: {SYSTEM_STATS['entity_links_discovered']}")

        def on_clear_cache():
            cache_size = clear_query_cache()
            return f"Cache: {len(QUERY_CACHE)} entries (cleared {cache_size})"

        # Set up event listeners
        submit_btn.click(
            fn=on_submit,
            inputs=[question_input, chatbot],
            outputs=[chatbot, question_input, detected_entities, query_output, results, conversation_complexity, 
                    entity_relationships, system_info, cache_status]
        )
        ...
        clear_history_btn.click(
            fn=clear_history,
            inputs=[],
            outputs=[chatbot, detected_entities, query_output, results, conversation_complexity, 
                    entity_relationships, system_info, cache_status]
        )

        # Connect cache clear button
        ...
        gr.Examples(
            examples=[
                ...
                "Show me which suppliers have worked on multiple projects and list the projects",  # Relationship question
                "What's the trend of purchase amounts for Al Jaber Transport over the past year?",  # Trend analysis
            ],
...
def system_stats():
    ...
    html += f"<p>Successful Recoveries: {SYSTEM_STATS['successful_recoveries']}</p>"
    html += f"<p>Decomposed Questions: {SYSTEM_STATS['decomposed_questions']}</p>"
    html += f"<p>Entity Links Discovered: {SYSTEM_STATS['entity_links_discovered']}</p>"
    html += f"<p>Cache Size: {len(QUERY_CACHE)} entries</p>"
    return HTMLResponse(content=html)

@app.get("/clear-cache", response_class=HTMLResponse)
def clear_cache():
    cache_size = clear_query_cache()
    return HTMLResponse(content=f"<h2>Cache Cleared</h2><p>Removed {cache_size} entries from cache</p>")

if __name__ == "__main__":
    initialize_unique_lists()
    interface = create_interface()
    app = gr.mount_gradio_app(app, interface, path="/")
    # Use port 7871 for rev.18 to avoid conflict with rev.17 on port 7870
    webbrowser.open("http://localhost:7871")
