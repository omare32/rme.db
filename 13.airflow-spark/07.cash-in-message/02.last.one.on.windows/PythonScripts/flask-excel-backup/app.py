...
def fetch_data():
    ...
    try:
        ...
        query = f"""
            SELECT 
                CAST(RECEIPT_ID AS CHAR) AS RECEIPT_ID,  -- Force ID to be text at the database level
                RECEIPT_NUMBER, 
                CUSTOMER_NO, 
                CUSTOMER_NAME, 
                RECEIPT_PRJ_CODE, 
        ...
        # ✅ Ensure "RPT_ ID" is treated as pure text from the database level (CAST as CHAR in SQL)
        df["RPT_ ID"] = df["RPT_ ID"].astype(str)  # Ensure it's string in Pandas

        # ✅ Format "RPT_Date" to "DD-MMM-YY"
        ...
        with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
            ...
            # Format for dates: "DD-MMM-YY" (e.g., "31-Aug-17")
            date_format = workbook.add_format({"num_format": "DD-MMM-YY"})

            # ✅ Force "RPT_ ID" to be text in Excel using cell formatting (instead of modifying data)
            ...
            for col_num, col_name in enumerate(df.columns):
                ...
                if col_name == "RPT_ ID":
                    worksheet.set_column(col_num, col_num, col_width, text_format)  # ✅ Force Excel to treat it as text
                
                elif col_name == "RPT_Date":
                    worksheet.set_column(col_num, col_num, col_width, date_format)  # ✅ Date formatting
                
                elif pd.api.types.is_numeric_dtype(df[col_name]):
                    worksheet.set_column(col_num, col_num, col_width, number_format)  # ✅ Number formatting
                
                else:
                    worksheet.set_column(col_num, col_num, col_width)  # General text formatting
