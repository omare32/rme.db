...
# ✅ WhatsApp Recipients (Ensure they're registered in Twilio sandbox)
whatsapp_recipients = [
...
# ✅ File paths for storing last transactions
...
try:
    ...
    # ✅ Fetch latest cash-in transaction
    ...
    if result:
        ...
        project_name = project_mapping.get(project_number, None)  # Fetch project name
        ...
        # ✅ Read last transaction from file
        last_transaction = read_file(last_cash_in_file)

        if current_transaction != last_transaction:
            print("New transaction found! Preparing to send notifications...")
            ...
            http_client.session.verify = False  # Bypass SSL Verification
