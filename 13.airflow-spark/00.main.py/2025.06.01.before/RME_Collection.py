...
def Collection_ETL():
    ...
    RES = fx.connection(spark,'RES','RMEDB',Collection_query,'TEMP','ERP')
...
dag = DAG('Collection',catchup=False,default_args=default_args,schedule_interval='30 3 * * *',tags=['4'])


CollectionTask= PythonOperator(dag=dag,
                task_id = 'Collection',
                python_callable=Collection_ETL) 
