...
def SWD_Collection_ETL():
    spark = fx.spark_app('SWD_Collection','4g','4')
    RES = fx.connection(spark,'RES','RMEDB',SWD_Collection_query,'TEMP','ERP')
    fx.WriteFunction(RES ,load_connection_string,'SWD_Collection_Report','overwrite',conn.mysql_username,conn.mysql_password)    
...
dag = DAG('SWD_Collection',catchup=False,default_args=default_args,schedule_interval='30 4 * * *',tags=['4'])


SWD_CollectionTask= PythonOperator(dag=dag,
                task_id = 'SWD_Collection',
                python_callable=SWD_Collection_ETL) 
