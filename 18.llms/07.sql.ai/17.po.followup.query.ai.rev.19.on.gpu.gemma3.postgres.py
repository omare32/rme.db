...
def process_question(question: str, use_history: bool = True) -> Tuple[str, str, List[str], List[List], Dict]:
    ...
    print("[DEBUG] LLM extracted project:", llm_entities.get("project"), "supplier:", llm_entities.get("supplier"))
    print("[DEBUG] UNIQUE_PROJECTS sample:", UNIQUE_PROJECTS[:10])
    print("[DEBUG] UNIQUE_SUPPLIERS sample:", UNIQUE_SUPPLIERS[:10])
    ...
    if llm_entities is not None:
        ...
        if "project" in llm_entities and llm_entities["project"] is not None:
            ...
            print("[DEBUG] Trying to match project_candidate:", project_candidate)
            # Exact match
        ...
        if "supplier" in llm_entities and llm_entities["supplier"] is not None:
            ...
            print("[DEBUG] Trying to match supplier_candidate:", supplier_candidate)
            # Exact match
