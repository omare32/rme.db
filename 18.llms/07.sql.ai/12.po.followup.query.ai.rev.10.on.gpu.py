...
# Ollama API will be called locally on the GPU machine
...
def generate_sql_query(question: str) -> str:
    """Use Ollama Llama4 to convert natural language question to SQL query"""
    try:
        ...
        # Llama-4 returns the answer in response_data['choices'][0]['message']['content']
...
def generate_natural_language_answer(question: str, sql_query: str, columns: list, results: list) -> str:
    """Ask Llama-4 to describe the SQL result in English."""
    # Format results as markdown table (or plain text if empty)
...
def process_question(question: str) -> Tuple[str, str, List[str], List[List]]:
    """Process a natural language question and return LLM answer, SQL, and results"""
...
def create_interface():
    ...
    with gr.Blocks(title="RME PO Query Assistant rev.10 (Ollama Local)") as interface:
        gr.Markdown("""
        # PO Follow-Up Query AI (rev.10, Ollama Llama4 Local)
        This AI assistant can help you query the Purchase Order database using natural language.\n\n**Note:** This system uses a table with essential PO information, including payment/delivery terms.\n\n**Powered by Ollama Llama4 running on your local GPU server!**
        """)
        with gr.Row():
