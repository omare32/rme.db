...
if __name__ == "__main__":
    ...
    demo.launch(share=False, debug=True)  # Set share=False to avoid the frpc issue THEN CAST(amount AS NUMERIC) ELSE 0 END) as total_amount,
                AVG(CASE WHEN amount ~ '^[0-9]+\.?[0-9]*
...
if __name__ == "__main__":
    ...
    print("\nLaunching Gradio interface...")
    demo = create_gradio_interface()
    demo.launch(share=True, debug=True) THEN CAST(amount AS NUMERIC) ELSE NULL END) as avg_amount,
...
if __name__ == "__main__":
    ...
    print("\nLaunching Gradio interface...")
    demo = create_gradio_interface()
    demo.launch(share=True, debug=True) THEN CAST(amount AS NUMERIC) ELSE 0 END) as total_amount,
                currency
