...
def extract_entities_with_regex(question: str) -> Dict:
    """Enhanced regex-based entity extraction with better patterns"""
    entities = {"project": None, "supplier": None, "confidence": 0.0}
    
    question_lower = question.lower().strip()
    
    # Enhanced project patterns - more comprehensive
    project_patterns = [
        r'\bfor\s+(.+?)\s+project\b',          # "for ring road project"
        r'\bin\s+(.+?)\s+project\b',           # "in ring road project" 
        r'\bproject\s+(.+?)(?:\s|$)',          # "project ring road"
        r'\b(.+?)\s+project\b',                # "ring road project"
        r'\bproject\s*[:=]\s*(.+?)(?:\s|$)',   # "project: ring road"
        r'\bof\s+(.+?)\s+project\b',           # "of ring road project"
        r'\bon\s+(.+?)\s+project\b',           # "on ring road project"
    ]
    
    # Enhanced supplier patterns
    supplier_patterns = [
        r'\bfrom\s+(.+?)(?:\s|$)',             # "from siemens"
        r'\bsupplier\s+(.+?)(?:\s|$)',         # "supplier siemens"
        r'\bvendor\s+(.+?)(?:\s|$)',           # "vendor siemens"
        r'\bby\s+(.+?)(?:\s|$)',               # "by siemens"
        r'\bwith\s+(.+?)(?:\s|$)',             # "with siemens" (context dependent)
        r'\b(.+?)\s+orders\b',                 # "siemens orders"
        r'\b(.+?)\s+supplier\b',               # "siemens supplier"
    ]
    ...
    for pattern in project_patterns:
        ...
        if match:
            ...
            # Clean up common words that shouldn't be part of project name
            candidate = re.sub(r'\b(the|a|an|and|or|in|on|at|for|with|by)\b', '', candidate).strip()
            if candidate and len(candidate) >= 2:
                entities["project"] = candidate
                entities["confidence"] = 0.8
                print(f"[REGEX] Found project candidate: '{candidate}' using pattern: {pattern}")
                break
    ...
    for pattern in supplier_patterns:
        ...
        if match:
            candidate = match.group(1).strip()
            # Filter out common words that are not supplier names
            skip_words = {'the', 'a', 'an', 'all', 'any', 'some', 'orders', 'pos', 'purchase', 'highest', 'amount'}
            if candidate and candidate not in skip_words and len(candidate) >= 2:
                entities["supplier"] = candidate
                entities["confidence"] = max(entities["confidence"], 0.8)
                print(f"[REGEX] Found supplier candidate: '{candidate}' using pattern: {pattern}")
                break
    
    return entities

# --- Enhanced LLM Entity Detection ---
...
Your task: Extract project names and supplier/vendor names from the user's question.
...
EXTRACTION Rules:
1. Extract project names when mentioned with words like: "project", "for", "in", "on"
2. Extract supplier/vendor names when mentioned with words like: "from", "supplier", "vendor", "by"
3. Handle variations like "ring road project" should extract "ring road" as project
4. For "highest amount in ring road project" - extract "ring road" as project
5. Return the cleaned entity name without extra words like "project", "supplier"
6. Be case-insensitive but return proper case

Context: {context}

Return ONLY a JSON object with this exact format:
{{"project": "entity_name_or_null", "supplier": "entity_name_or_null", "confidence": 0.9}}

Examples:
Q: "What's the supplier with the highest amount in ring road project?"
...
Q: "JSW Steel orders for highway project"
A: {{"project": "highway", "supplier": "JSW Steel", "confidence": 0.9}}"""
...
def fuzzy_match_entity(candidate: str, entity_list: list, threshold: float = 0.6) -> Tuple[Optional[str], float]:
    ...
    for entity in entity_list:
        ...
        # Strategy 2: Partial match (candidate is part of entity or vice versa)
        if candidate_clean in entity_clean or entity_clean in candidate_clean:
            # Calculate similarity score
            shorter = min(len(candidate_clean), len(entity_clean))
            longer = max(len(candidate_clean), len(entity_clean))
            score = shorter / longer if longer > 0 else 0
            if score > best_score and score > 0.4:  # Lower threshold for partial matches
        ...
        if ratio > best_score and ratio > 0.6:  # Slightly lower threshold
        ...
        if len(candidate_tokens) > 0 and len(entity_tokens) > 0:
            intersection = candidate_tokens.intersection(entity_tokens)
            if len(intersection) >= 1:  # At least 1 common word
                union = candidate_tokens.union(entity_tokens)
                jaccard = len(intersection) / len(union) if union else 0
                # Boost score if it's a good token match
                if jaccard > 0.3 and jaccard > best_score * 0.8:
...
def extract_entities_from_question(question: str) -> Dict:
    """Main entity extraction function combining multiple approaches"""
    ...
    # Strategy 1: Try regex-based extraction first (faster and often more accurate)
    print("[REGEX] Attempting regex-based entity extraction...")
    regex_result = extract_entities_with_regex(question)
    
    # Strategy 2: Try LLM extraction as backup/enhancement
    print("[LLM] Attempting LLM-based entity extraction...")
    llm_result = detect_entities_with_llm(question, context)
    
    # Combine results - prefer regex if confident, otherwise use LLM
    final_project = None
    final_supplier = None
    
    # Process project
    project_candidates = []
    if regex_result.get("project"):
        project_candidates.append((regex_result["project"], regex_result["confidence"], "regex"))
    if llm_result.get("project"):
        project_candidates.append((llm_result["project"], llm_result["confidence"], "llm"))
    
    for candidate, confidence, method in project_candidates:
        matched_project, match_score = fuzzy_match_entity(candidate, UNIQUE_PROJECTS)
        if matched_project:
            final_confidence = confidence * match_score
            print(f"[{method.upper()}] Project candidate '{candidate}' matched to '{matched_project}' with confidence {final_confidence}")
            if final_confidence > detected_entities["project_confidence"]:
                detected_entities["project"] = matched_project
                detected_entities["project_confidence"] = final_confidence
                detected_entities["method"] = f"{method}+fuzzy"
    
    # Process supplier
    supplier_candidates = []
    if regex_result.get("supplier"):
        supplier_candidates.append((regex_result["supplier"], regex_result["confidence"], "regex"))
    if llm_result.get("supplier"):
        supplier_candidates.append((llm_result["supplier"], llm_result["confidence"], "llm"))
    
    for candidate, confidence, method in supplier_candidates:
        matched_supplier, match_score = fuzzy_match_entity(candidate, UNIQUE_SUPPLIERS)
        if matched_supplier:
            final_confidence = confidence * match_score
            print(f"[{method.upper()}] Supplier candidate '{candidate}' matched to '{matched_supplier}' with confidence {final_confidence}")
            if final_confidence > detected_entities["supplier_confidence"]:
                detected_entities["supplier"] = matched_supplier
                detected_entities["supplier_confidence"] = final_confidence
                detected_entities["method"] = f"{method}+fuzzy"
...
if __name__ == "__main__":
    ...
    test_questions = [
        ...
        "What's the supplier with the highest amount in ring road project?",
