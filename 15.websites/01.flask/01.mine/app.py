...
def fetch_data():
    """Fetches selected columns from MySQL and saves it as an Excel file."""
    try:
        cnx = mysql.connect(**DB_CONFIG)
        query = f"""
            SELECT RECEIPT_ID, RECEIPT_PRJ_NAME, RECEIPT_PRJ_CODE, 
                   RECEIPT_AMOUNT, RECEIPT_DATE, INV_NUM 
            FROM {TABLE_NAME}
        """
