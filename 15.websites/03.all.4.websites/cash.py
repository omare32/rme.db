...
try:
    ...
    projects_df = pd.read_sql("SELECT PROJECT_NUM, PROJECT_NAME, SECTOR FROM PROJECT_SECTOR_MAP", cnx)
    dropdown_sectors = sorted(projects_df['SECTOR'].dropna().unique())
    cnx.close()
except Error as e:
    print(f"❌ Failed to load sector list: {e}")
    projects_df = pd.DataFrame(columns=["PROJECT_NUM", "PROJECT_NAME", "SECTOR"])
    dropdown_sectors = []
        ...
        <form method="post">
            <div class="row g-3">
                ...
                <div class="col-md-2 align-self-end">
                    <button type="submit" class="btn btn-success w-100">Show</button>
                </div>
            </div>
        </form>
...
def get_all_sums_for_sector(sector, from_date=None, to_date=None):
    metric_map = {
        'COST': {'table': 'RME_Projects_Cost_Dist_Line_Report', 'amount_col': 'AMOUNT', 'project_col': 'PROJECT_NUM', 'date_col': 'GL_DATE'},
        'CASH_OUT': {'table': 'RME_ap_check_payments_Report', 'amount_col': 'EQUIV', 'project_col': 'PROJECT_NUMBER', 'date_col': 'CHECK_DATE'},
        'CASH_IN': {'table': 'SWD_Collection_Report', 'amount_col': 'FUNC_AMOUNT', 'project_col': 'PROJECT_NUM', 'date_col': 'RECEIPT_DATE'}
    }
...
# ✅ Run the app (bind to external IP)
