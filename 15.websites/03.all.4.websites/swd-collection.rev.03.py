...
def index():
    try:
        ...
        # Fetch project names where PROJECT_NUM is a 4-digit string between '0001' and '0227'
        query_projects = f"""
            SELECT DISTINCT PROJECT_NAME
            FROM {TABLE_NAME}
            WHERE PROJECT_NUM REGEXP '^[0-9]{4}$' AND PROJECT_NUM >= '0001' AND PROJECT_NUM <= '0227'
        ...
        logging.info(f"Fetched {len(project_names)} filtered project names from the database.")

        return render_template('index_swd.html', projects=project_names, show_all_option=False)

    except Exception as e:
        logging.error(f"Error fetching dropdown data: {e}")
        return render_template('index_swd.html', projects=[], show_all_option=False)
    ...
    try:
        ...
        query = f"""
            ...
            WHERE PROJECT_NAME = %s
                AND PROJECT_NUM REGEXP '^[0-9]{4}$' AND PROJECT_NUM >= '0001' AND PROJECT_NUM <= '0227'
        """
...
if __name__ == '__main__':
    app.run(debug=True, host="0.0.0.0", port=5002)  # Run on port 5002 for the new filtered SWD page 