...
def main():
    ...
    try:
        ...
        cost_df = cost_df_full[['PROJECT_NAME', 'VENDOR_NAME', 'AMOUNT', 'GL_DATE']].copy()
        ...
        cash_df = cash_df_full[['PROJECT_NAME', 'Supplier Name', 'AMOUNT', 'CHECK_DATE']].copy()
        cash_df = cash_df.rename(columns={'Supplier Name': 'VENDOR_NAME'})
        print(f"Fetched {len(cash_df)} cash records")

        # Export full datasets
        print("\nExporting complete datasets...")
        cost_df_full.to_excel(os.path.join(output_dir, 'complete_cost_data_2025.xlsx'), index=False)
        cash_df_full.to_excel(os.path.join(output_dir, 'complete_cash_data_2025.xlsx'), index=False)
