...
def search_cvs():
    ...
    try:
        ...
        keyword_conditions = " OR ".join([
            "LOWER(ocr_result) LIKE LOWER(%s) OR "
            "LOWER(skills) LIKE LOWER(%s) OR "
            "LOWER(university) LIKE LOWER(%s)"
        ...
        for keyword in keyword_list:
            params.extend([f"%{keyword}%"] * 3)  # 3 times for each field we're searching

        # Connect to database and execute search
        with get_db_connection() as conn:
            with conn.cursor() as cur:
                query = f"""
                    SELECT id, pdf_filename, name, email, phone, linkedin,
                           graduation_year, university, skills
