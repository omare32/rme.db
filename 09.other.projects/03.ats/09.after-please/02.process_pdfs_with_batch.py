...
def process_pdfs_in_directory(directory_path, batch_size=10):
    ...
    try:
        ...
        for i in range(0, len(unprocessed_files), batch_size):
            ...
            for pdf_file in batch:
                ...
                if ocr_text:
                    batch_cost += estimate_token_cost(ocr_text)
            
            print(f"Estimated cost for this batch: ${batch_cost:.3f}")
            
            # Ask for confirmation
            response = input(f"Process this batch? (y/n): ").lower().strip()
            if response != 'y':
            ...
            # Ask to continue with next batch
