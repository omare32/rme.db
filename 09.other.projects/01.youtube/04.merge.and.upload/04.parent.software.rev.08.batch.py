...
def merge_videos_in_batches(video_files: List[Tuple[str, str]], output_path: str, batch_size: int = 5, progress_window: Optional[ProgressWindow] = None) -> bool:
    ...
    try:
        ...
        for batch_num, batch in enumerate(batches, 1):
            ...
            # Merge batch with progress monitoring
            cmd = [
                "ffmpeg",
                "-hide_banner",
                "-loglevel", "info",
            ...
            while True:
                ...
                if error_line:
                    print(f"FFmpeg error: {error_line.strip()}")
                    error_output.append(error_line)
                
                # Check for progress timeout
                if time.time() - last_progress_time > 300:  # 5 minutes timeout
        ...
        if len(batch_outputs) == 1:
            ...
            while True:
                ...
                error_line = process.stderr.readline()
                if error_line:
                    print(f"Final merge error: {error_line.strip()}")
                    error_output.append(error_line)
